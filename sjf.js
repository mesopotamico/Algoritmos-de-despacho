...
function sjf(tl, tr) {
    ...
    while (completed !== n) {
        ...
        let min_index = -1;
        for (let i = 0; i < n; i++) {
            if (tl[i] <= t_eje && tr[i] <= min_burst && proccess[i] !== 0) {
                if (tr[i] < min_burst || (tr[i] === min_burst && tl[i] < tl[min_index])) {
                    min_burst = tr[i];
                    min_index = i;
                }
            }
        ...
        if (min_index === -1) {
            ...
            procesos_ordenados[completed] = min_index + 1; //Tener en cuenta para la gf
...
function guardarTiempos() {
    ...
    if (!errores) {
        sjf(tiempoLlegada, tiempoRafaga);
    }
}
